...
DEBIAN_FRONTEND=noninteractive apt-get install -y curl sudo rsyslog iptables iptables-persistent libcurl4-openssl-dev wget build-essential postfix postfix-mysql postfix-pcre procmail apache2 mysql-server git gnupg openssh-server openssl python-virtualenv python-pip python-lxml python-dev libjpeg-dev tor torsocks dnsutils python-dev libxml2-dev libxslt1-dev zlib1g-dev python-jinja2 python-lxml python-pgpdump spambayes
if [ "$?" -ne "0" ]; then
exit 1;
fi
